...
const Converter = () => {
  ...
  const bases = {
    decimal: { name: "十进制", radix: 10 },
    binary: { name: "二进制", radix: 2 },
    octal: { name: "八进制", radix: 8 },
    hexadecimal: { name: "十六进制", radix: 16 },
  };
  ...
  const examples = {
    binary: "示例: 1010.101 (十进制的 10.625 的二进制表示)",
    octal: "示例: 12.4 (十进制的 10.5 的八进制表示)",
    decimal: "示例: 10.5 (标准十进制格式)",
    hexadecimal: "示例: A.8 (十进制的 10.5 的十六进制表示)",
  };

  const validateInput = (value, baseType) => {
    if (!value.trim()) {
      throw new Error("⚠️ 请输入一个数字。");
    }

    if (!baseValidation[baseType].test(value)) {
      throw new Error(
        `❌ 输入无效：${bases[baseType].name}。${examples[baseType]}`
      );
    }
  };
    ...
    try {
      ...
      switch (operation) {
        ...
        case "multiply":
          ...
          break;
        case "divide":
          if (num2 === 0) throw new Error("不允许除以零");
          calculatedValue = num1 / num2;
          break;
        default:
          ...
          <>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full p-3 border border-gray-300 rounded-lg text-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent"
                placeholder={`输入 ${bases[fromBase].name} 数字...`}
                aria-label="输入数字"
              />
            </div>
          </>
          ...
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            {activeTab === "converter" && (
              <p className="mt-2 text-sm text-gray-500">
                {bases[fromBase].name} 到 {bases[toBase].name} 转换
              </p>
            )}
            {activeTab === "calculator" && (
              <p className="mt-2 text-sm text-gray-500">
